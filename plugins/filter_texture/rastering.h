...
class VertexSampler
{
    ...
    typedef vcg::tri::FaceTmark<VCGMesh> MarkerFace;
    ...
    vcg::face::PointDistanceBaseFunctor<VCGMesh::ScalarType> PDistFunct;
...
public:
	VertexSampler(VCGMesh &_srcMesh, std::vector <QImage> &_srcImg, float upperBound) :
    ...
    void AddVert(VCGVertex &v)
    {
        // Get Closest point
        VCGMesh::CoordType closestPt;
        VCGMesh::ScalarType dist=dist_upper_bound;
        VCGFace *nearestF;
        ...
        if (ret) {
            ...
            if ((tIndex >= 0) && ((size_t)tIndex < srcImgs.size()))
            {
                ...
                v.C() = VCGVertex::ColorType(qRed(px), qGreen(px), qBlue(px), 255);
            }
            else
            {
...
public:
    ...
    void AddTextureSample(const VCGFace &f, const VCGMesh::CoordType &p, const vcg::Point2i &tp, float edgeDist= 0.0)
    {
        VCGVertex::ColorType c;
        /*int alpha = 255;
...
{
    typedef vcg::GridStaticPtr<VCGFace, VCGMesh::ScalarType > MetroMeshGrid;
    typedef vcg::GridStaticPtr<VCGVertex, VCGMesh::ScalarType > VertexMeshGrid;
...
public:
    TransferColorSampler(VCGMesh &_srcMesh, std::vector <QImage> &_trgImgs, float upperBound, int _vertexMode)
    : trgImgs(_trgImgs), dist_upper_bound(upperBound)
    {
        ...
        if(vertexMode==2)
        {
            std::pair<float,float> minmax = vcg::tri::Stat<VCGMesh>::ComputePerVertexQualityMinMax(_srcMesh);
            minQ=minmax.first;
            maxQ=minmax.second;
        }
    }

	TransferColorSampler(VCGMesh &_srcMesh, std::vector <QImage> &_trgImgs, std::vector <QImage> *_srcImgs, float upperBound)
    ...
    void AddTextureSample(const VCGFace &f, const VCGMesh::CoordType &p, const vcg::Point2i &tp, float edgeDist=0.0)
    {
        ...
        {
            VCGVertex   *nearestV=0;
            VCGMesh::ScalarType dist=dist_upper_bound;
            nearestV =  vcg::tri::GetClosestVertex<VCGMesh,VertexMeshGrid>(*srcMesh,unifGridVert,startPt,dist_upper_bound,dist); //(PDistFunct,markerFunctor,startPt,dist_upper_bound,dist,closestPt);
		...
		if (alpha == 255 || qAlpha(targetImage.pixel(tx, ty)) < alpha)
        {
            ...
            else
            {
                ...
                case 0 : // Color
                    ...
                    break;
                case 1 : { // Normal
                    VCGMesh::CoordType nn = nearestF->V(0)->cN()*interp[0]+
                                           nearestF->V(1)->cN()*interp[1]+
                                           nearestF->V(2)->cN()*interp[2];
                    nn.Normalize();
                    nn= ((nn+VCGMesh::CoordType(1.0,1.0,1.0))/2.0f)*255.0f;

...
public:
	...
	// Least-squares objective over Equi_energy's two residuals, so the patch
	// optimisation can run on newuoa (bundled in vcglib) instead of levmar. Equi_energy
	// reports a folded parametrization as FLT_MAX residuals; squaring those overflows, so
	// a folded configuration is reported as one large finite penalty instead.
	struct EquiObjective
	{
		void *data;
		double operator()(int /*n*/, const double *p) const
		{
			float pf[2] = {float(p[0]), float(p[1])};
			float x[2] = {0.f, 0.f};
			Equi_energy(pf, x, 2, 2, data);
			const double kFolded = 1e18;
			if (!std::isfinite(x[0]) || !std::isfinite(x[1])
			    || std::fabs(x[0]) >= std::numeric_limits<float>::max()
			    || std::fabs(x[1]) >= std::numeric_limits<float>::max())
				return kFolded;
			return double(x[0]) * double(x[0]) + double(x[1]) * double(x[1]);
		}
	};
   ...
   static void OptimizeUV(VertexType *center,MeshType &base_domain)
	...
	/*	assert(Test(ordFaces,faces));
		...
		// Was slevmar_dif(Equi_energy, p, x, 2, 2, 1000, ...) -- two parameters, two
		// residuals, finite-difference Jacobian. newuoa is derivative-free too and is
		// already vendored with vcglib, so this drops levmar's GPL download entirely.
		{
			EquiObjective objective{&Minf};
			double pd[2] = {double(p[0]), double(p[1])};
			min_newuoa<double, EquiObjective>(2, pd, objective, 1.0, 1e-8, 1000);
			p[0] = float(pd[0]);
			p[1] = float(pd[1]);
			// Leave the optimum installed: Equi_energy writes through to the vertex.
			float xf[2];
			Equi_energy(p, xf, 2, 2, &Minf);
			x[0] = xf[0];
			x[1] = xf[1];
		}

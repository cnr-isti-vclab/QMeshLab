...
//    return LinkConditions(Super::pos);
  ...
  // Least-squares objective over energy0's four residuals, so this runs on newuoa
  // (bundled in vcglib) rather than levmar. Same treatment as the patch optimiser in
  // opt_patch.h: an unusable configuration comes back as one large finite penalty.
  struct Energy0Objective
  {
    void *data;
    double operator()(int /*n*/, const double *p) const
    {
      ...
      energy0(pd, x, 3, 4, data);
      double sum = 0.0;
      for (double r : x) {
        if (!std::isfinite(r) || std::fabs(r) >= std::numeric_limits<double>::max())
          return 1e18;
        sum += r * r;
      }
      return sum;
...
//	DoCollapse(created,posEdge, newPos); // v0 is deleted and v1 take the new position
  EdgeCollapser<BaseMesh,VertexPair>::Do(created, posEdge, newPos);
    ...
    // Was dlevmar_dif(energy0, p, x, 3, 4, 1000, ...): three parameters, four residuals.
    {
      Energy0Objective objective{&Minf};
      min_newuoa<double, Energy0Objective>(3, p, objective, 1.0, 1e-8, 1000);
      // Leave the optimum installed: energy0 writes through to the vertex it moves.
      energy0(p, x, 3, 4, &Minf);
    }

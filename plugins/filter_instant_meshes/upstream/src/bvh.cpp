    ...
    void execute() {
        ...
        tasks.run([&] {
            BVHBuildTask(bvh, node_idx_right, start + left_count,
                         end, temp + left_count).execute();
        });
        BVHBuildTask(bvh, node_idx_left, start,
                     start + left_count, temp).execute();
        tasks.wait();
    }
...
void BVH::build(const ProgressCallback &progress) {
    ...
    BVHBuildTask(*this, 0u, mIndices, mIndices + total_size, temp).execute();
    delete[] temp;

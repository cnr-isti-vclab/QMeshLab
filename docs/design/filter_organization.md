...
|---|---|---|---|
| `filter_basic` | 3 | no dep; three unrelated filters | `filter_measure` (Current Mesh Info), `filter_transform` (Normalize To Unit Box), `filter_create` (Noisy Isosurface) |
| `filter_create` | 13 | already a family name | `filter_create` (unchanged) |
| `filter_clean` | 15 | already a family name | `filter_clean` (14), `filter_reconstruct` (1 — its lone non-Cleaning filter is *Surface Reconstruction: Ball Pivoting*) |
...
| `filter_plymc` | 2 | no dep — uses vcglib's own `create/plymc/plymc.h` | `filter_reconstruct` (1 *Surface Reconstruction: VCG*), `filter_remesh` (1 *Edge Collapse for Marching Cube meshes*) |
